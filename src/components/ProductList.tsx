  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-text-main tracking-tight">Ürün Yönetimi</h2>
          <p className="text-sm text-text-muted">{products.length} toplam ürün listeleniyor.</p>
        </div>
        <button 
          onClick={onAddProduct}
          className="btn-primary px-6 py-2.5 flex items-center justify-center h-11"
        >
      ...
      {/* Filters Bar */}
      <div className="card p-4 flex flex-wrap items-center gap-4 bg-white shadow-sm">
        <div className="relative flex-1 min-w-[280px]">
          ...
          <input 
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-bg-main rounded-lg text-sm border border-border-color focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all outline-none"
          />
        </div>

        <div className="flex items-center space-x-3">
          <div className="flex items-center px-3 py-2 bg-bg-main rounded-lg border border-border-color">
            <Filter className="w-4 h-4 text-text-muted mr-2" />
            <select 
              value={filterCategory}
              onChange={(e) => setFilterCategory(e.target.value)}
              className="bg-transparent text-sm font-semibold outline-none cursor-pointer text-text-main"
            >
              {categories.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
          </div>

          <div className="flex bg-bg-main rounded-lg p-1 border border-border-color">
            <button 
              onClick={() => setViewMode('grid')}
              className={cn("p-1.5 rounded-md transition-all", viewMode === 'grid' ? "bg-white shadow-sm text-primary" : "text-text-muted")}
            >
              <LayoutGrid className="w-4 h-4" />
            </button>
            <button 
              onClick={() => setViewMode('table')}
              className={cn("p-1.5 rounded-md transition-all", viewMode === 'table' ? "bg-white shadow-sm text-primary" : "text-text-muted")}
            >
          ...
          {filteredProducts.map((p) => (
            ...
            >
              <div className="aspect-square bg-bg-main relative">
                 {p.cover_image ? (
                   <img src={p.cover_image} className="w-full h-full object-contain group-hover:scale-105 transition-transform duration-500 p-4" />
                 ) : (
              ...
              <div className="p-4">
                <p className="text-[10px] font-bold text-text-muted uppercase tracking-widest mb-1">{p.category}</p>
                ...
                <div className="mt-4 pt-4 border-t border-border-color flex items-center justify-between">
                   ...
                   <div className="text-right">
                     <p className={cn(
                       "text-xs font-bold", 
                       (p.total_stock || 0) < (parseInt(localStorage.getItem('settings') ? JSON.parse(localStorage.getItem('settings')!!).low_stock_threshold : '10')) ? "text-danger" : "text-success"
                     )}>
                ...
                {filteredProducts.map((p) => (
                  <tr key={p.id} onClick={() => onProductClick(p.id)} className="hover:bg-bg-main cursor-pointer group transition-colors">
                    <td className="px-6 py-5">
                      <div className="w-12 h-12 rounded-xl bg-bg-main border border-border-color overflow-hidden p-1.5 flex items-center justify-center shadow-sm bg-white">
                        {p.cover_image ? (
                          <img src={p.cover_image} className="w-full h-full object-contain" />
                        ) : (
                    ...
                    <td className="px-6 py-5">
                      <p className="text-sm font-bold text-text-main group-hover:text-primary transition-colors">{p.name || p.title}</p>
                      <p className="text-[10px] text-text-muted font-mono mt-0.5 uppercase tracking-tighter">{p.sku}</p>
                    </td>
                    <td className="px-6 py-5">
                    ...
                    <td className="px-6 py-5 text-sm font-extrabold text-text-main">{formatCurrency(p.sale_price)}</td>
